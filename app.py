...
scoreboard=lambda: {"Team A":0,"Team B":0}
# display scores
d={"Team A":0,"Team B":0}
for day,ms in matches.items():
    for idx,(p1,p2) in enumerate(ms):
        ...
        pts=rec.get(key,{})
        if len(p1)==1:
            d["Team A"]+=pts.get(p1[0],0); d["Team B"]+=pts.get(p2[0],0)
        else:
...
for i,tab in enumerate(tabs, start=1):
    with tab:
        ...
        # per-day totals
        tot={"Team A":0,"Team B":0}
        for idx,(p1,p2) in enumerate(matches[i]):
            rec=scores_col.find_one({"day":i,"match_index":idx}) or {}
            key="total_points" if len(p1)==1 else"team_points"
            pts=rec.get(key,{})
            if len(p1)==1:
                tot["Team A"]+=pts.get(p1[0],0); tot["Team B"]+=pts.get(p2[0],0)
            else:
                tot["Team A"]+=pts.get("Team A",0); tot["Team B"]+=pts.get("Team B",0)
        st.write(f"**Totals:** A {tot['Team A']} — B {tot['Team B']}")
        # matches
        for idx,(p1,p2) in enumerate(matches[i]):
            with st.expander(f"Match {idx+1}: {' & '.join(p1)} vs {' & '.join(p2)}"):
                ...
                raw=rec.get("hole_scores",{}); hole_scores={int(k):v for k,v in raw.items() if k.isdigit()}
                challenges=rec.get("challenges",[])
                # Clear match
                if st.button("Clear Match Scores",key=f"clear_{i}_{idx}"):
                    scores_col.delete_one({"day":i,"match_index":idx}); st.success(f"Cleared Match {idx+1}.")
                # hole entry
                ...
                if st.button("Save Hole Score",key=f"save_{i}_{idx}_{hole}"):
                    hole_scores[hole]=entry; pts=compute_points(hole_scores,p1,p2)
                    dbh={str(k):v for k,v in hole_scores.items()}
                    upd={"day":i,"match_index":idx,"players":(p1,p2),"hole_scores":dbh}
                    upd["total_points" if len(p1)==1 else"team_points"]=pts
                    scores_col.update_one({"day":i,"match_index":idx},{"$set":upd},upsert=True)
                ...
                if hole_scores:
                    ...
                    for h,sc in sorted(hole_scores.items()):
                        ...
                        if len(p1)==1:
                            rows.append({"Hole":h,p1[0]:sc[p1[0]],p2[0]:sc[p2[0]],f"{p1[0]} Pts":hpts[p1[0]],f"{p2[0]} Pts":hpts[p2[0]]})
                        else:
                            rows.append({"Hole":h,"Team A":sc["Team A"],"Team B":sc["Team B"],"Team A Pts":hpts["Team A"],"Team B Pts":hpts["Team B"]})
